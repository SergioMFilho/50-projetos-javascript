...
function calculaPorcentagemSorteio(assinante, minutosAssistidos) {
  let result = [];
  minutosAssistidos.forEach(function (element) {
    let resto = element % 60;
    if (resto > 0) {
      element += 60 - resto;
    }
    let hours = element / 60;
    result.push(hours);
  });

  let totalHours = 0;
  assinante.forEach(function (item, index) {
    if (item) {
      result[index] *= 2;
    }
    totalHours += result[index];
  });

  let resp = [];
  result.forEach(function (elem) {
    const prc = (elem * 100) / totalHours;
    resp.push(Math.round(prc));
  });
